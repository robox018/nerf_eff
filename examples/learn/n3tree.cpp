...
void unpack_llff_poses_bounds(cnpy::NpyArray& poses_bounds, float& width,
                              float& height, float& focal, bx::Vec3& up,
                              bx::Vec3& backward, bx::Vec3& cen) {
    ...
    for (size_t offs = 0; offs < poses_bounds.num_vals; offs += BLOCK_SZ) {
        for (size_t r = 0; r < 3; ++r) {
            ...
            right.raw[r] += row_ptr[1];
            up.raw[r] -= row_ptr[0];
            backward.raw[r] += row_ptr[2];
            cen.raw[r] += row_ptr[3];
        }
        bd_min =
    ...
    cen = div(cen , (total_cams * bd_min * 0.75f));
    backward = bx::normalize(backward);
    right = bx::normalize(bx::cross(up, backward));
    up = bx::normalize(bx::cross(backward, right));
}
...
N3Tree::N3Tree() : ndc_avg_up(0), ndc_avg_back(0), ndc_avg_cen(0) {}
N3Tree::N3Tree(const std::string& path) : ndc_avg_up(0), ndc_avg_back(0), ndc_avg_cen(0) { open(path); }
N3Tree::~N3Tree() {
#ifdef VOLREND_CUDA
	free_cuda();
#endif
